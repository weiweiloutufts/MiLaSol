...
def train_one_epoch(
    ...
    *,
    ...
    # ---- NEW ----
    epoch: int | None = None,
    z_save_dir: str | os.PathLike | None = None,
    save_labels_idx: bool = True,
):
    model.train()
    total_loss = (
        total_cls_loss
    ...
    for seqs, esm_embs, prot_embs, raygun_embs, solu_feats, labels, idx in train_loader:
        ...
        # ---- NEW: stash raw z / aug_z BEFORE normalize (or after; your call) ----
        # If you want the normalized embeddings instead, store `z_normal` later.
        z_list.append(z.detach().cpu())
        augz_list.append(aug_z.detach().cpu())
        if save_labels_idx:
            labels_list.append(labels.detach().view(-1).cpu())
            idx_list.append(idx.detach().view(-1).cpu())
    ...
    # ---- NEW: stack + save per epoch ----
    stacked_z = torch.cat(z_list, dim=0) if len(z_list) else torch.empty(0)
    stacked_augz = torch.cat(augz_list, dim=0) if len(augz_list) else torch.empty(0)
    
    if save_labels_idx and len(labels_list):
        stacked_labels = torch.cat(labels_list, dim=0)
        stacked_idx = torch.cat(idx_list, dim=0)
    else:
        stacked_labels = None
        stacked_idx = None
    
    if z_save_dir is not None:
        z_save_dir = Path(z_save_dir)
        z_save_dir.mkdir(parents=True, exist_ok=True)

        # epoch tag
        ep = int(epoch) if epoch is not None else 0

        torch.save(stacked_z, z_save_dir / f"Z_epoch{ep:03d}.pt")
        torch.save(stacked_augz, z_save_dir / f"augZ_epoch{ep:03d}.pt")

        if save_labels_idx and stacked_labels is not None:
            torch.save(stacked_labels, z_save_dir / f"labels_epoch{ep:03d}.pt")
            torch.save(stacked_idx, z_save_dir / f"idx_epoch{ep:03d}.pt")
    # Averages
...
def train(
    ...
    for epoch in range(num_epochs):
        ...
        logs = train_one_epoch(
            ...
            center_weight=0.1,
            epoch=epoch,
            z_save_dir="/cluster/home/wlou01/prot-solubility/data/latents",   
            save_labels_idx=True,
        )
